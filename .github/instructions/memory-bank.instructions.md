...
Important rule (project automation):
- Every time a todo list or task is created via agent actions, ALWAYS create a corresponding Markdown task file inside `memory-bank/tasks/` using the filename pattern `TASK{NNN}-{short-slug}.md` (for example `TASK009-add-download-tests.md`).
- Immediately update `memory-bank/tasks/_index.md` to reflect the new task and its initial status. The `_index.md` must list the task under the correct section (Pending, In Progress, Completed, or Abandoned).
- When updating progress on a task via the todo list tool, also append a progress log entry into the corresponding task Markdown file and update its `**Updated:**` date and `**Status:**` field accordingly.

This rule ensures the Memory Bank remains the single source of truth for tasks across agent restarts.
